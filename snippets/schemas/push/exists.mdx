If a schema with equivalent content already exists, a new schema is not created. However, the git metadata is added to the existing schema.

```bash output
$ miru schema push communication.yaml
🔍 Collecting Git Info...
    commit: df48a89cc19751f84cfbdc5908896235418a6719
    origin: git@github.com:miruml/getting-started.git
    file path: jsonschema/communication.yaml
    message: refactor: drop watchdog timeout from 5s to 2s

🟡 communication SCH-b82923a already exists
    language: jsonschema
    format: yaml
    created: Nov 20, 2025 at 3:45 PM PST
    instance file path: /communication.json
```