...
A parameter's path is a dot-separated list of keys starting with the config slug. Say we have the following config schema and config instance.
...
The parameter's path is `motion-control.joint-1.position.x`, which follows the format `<config-slug>.<config-instance-path>`.
...
<Tabs>
    <Tab title="Default">
        ...
        auto param = miru::config::query::get_param(
            ...
            config,
            // "<config-slug>.<config-instance-path>"
    ...
    <Tab title="ROS2 Style">
        ...
        miru::query::ROS2 ros2_config(config);
        auto param = ros2_config.get_parameter(
            // "<config-slug>.<parameter-path>"
            "motion-control.joint-1.position.x"
        );
