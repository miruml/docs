...
- First, fetch the [**config instance**](/docs/references/server-side-api/endpoints/config-instances/get) using its ID (assuming you already know it).
...
The `total_count` field in a list response represents the total number of items matching the query. Because Miru [paginates](/docs/references/server-side-api/query-params/pagination) list responses, this number is often greater than the number of items returned per page.
