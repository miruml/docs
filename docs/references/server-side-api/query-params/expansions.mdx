...
For example, a [config instance](/docs/references/server-side-api/endpoints/config-instances/get) has the following default response structure:
...
- First, fetch the [**config instance**](/docs/references/server-side-api/endpoints/config-instances/get) using the config instance ID (assuming you already know the ID).
- Then, having obtained the **`device_id`** from the config instance response, fetch the [**device**](/docs/references/server-side-api/endpoints/devices/get) using the device ID.
...
When listing config instances, you can expand fields in the same way you would for a single config instance. By default, [listing config instances](/docs/references/server-side-api/endpoints/config-instances/list) returns the following response structure.
...
The `total_count` field in a list response represents the total number of items matching the query. Because Miru [paginates](/docs/references/server-side-api/query-params/pagination) list responses, this number is often greater than the number of items actually returned in a single page.
