...
To make things simple, we've created a starter repository with some toy schemas and configs.

Follow along by cloning the [getting-started](https://github.com/miruml/getting-started) repository.

```bash
git clone https://github.com/miruml/getting-started.git
```

The getting-started repository contains a variety of schemas and instances. However, this guide focuses on the `Mobility` config type, which has two schema variations: 

- [Empty](https://github.com/miruml/getting-started/blob/main/schemas/mobility-schemaless.yaml) - an empty JSON Schema that regards all config instances as valid
- [Strict](https://github.com/miruml/getting-started/blob/main/schemas/mobility.yaml) - a well-defined JSON Schema that defines the valid fields, types, and values for instances of the `Mobility` config type
...
The slug is a unique identifier for the config type--it's how the CLI determines what config type a schema belongs to. As such, all config schemas must include the `x-miru-config-type` field.

We've already included the `x-miru-config-type` field in both the `mobility-schemaless.yaml` and `mobility.yaml` schemas inside the getting-started repository.
