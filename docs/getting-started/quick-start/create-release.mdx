...
Next, we'll install the CLI—the only method of creating releases in Miru.

This is a deliberate design choice. We believe a release's schemas should live in a Git repository. This allows them to be versioned alongside the code that uses them and encourages better software development practices.

While this tutorial covers creating releases on your local machine, the CLI can also be used to create releases in a [CI pipeline](/docs/learn/releases/create#continuous-integration).
...
For a more detailed guide on creating releases, visit the [create releases](/docs/learn/releases/create) page. For a comprehensive reference of the CLI command, visit the [CLI Reference](/docs/references/cli/release-create).