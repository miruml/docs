...
After installing, you'll need to retrieve the activation token for the desired client from the [Clients page](https://configs.miruml.com/clients) in the dashboard. Paste the token into the activation prompt in your terminal to complete setup.
...
Your application uses the Miru SDK and provides the path to the config schema file (e.g. `motion.schema.yaml`).
...
The resulting **concrete config** is then **spulled to the clientk** by the agent.
