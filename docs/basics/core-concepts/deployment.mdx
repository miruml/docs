...
2. The slug of the config type in the config schema
3. The device the agent is authenticated as

The SDK simply provides the config schema file to the agent, who can take care of the rest. The slug of the config type is defined within the config schema file and the agent knows which device it is authenticated as.
...
The Miru agent hashes the config schema's contents. This hash and the `motion-control` config type slug identify the `v8` version of the `Motion Control` config type (commit `8f4d2e1`) as the config schema used to render the config instance for `Robot A`.
...
## Networking & Caching 
It's expected that devices have unstable internet connection. The Miru agent caches config instances on the filesystem. Thus, even if the network is offline, the agent continues to serve the latest cached config instance to the SDK on the robot.
