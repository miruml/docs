...
and a history of [config instances](/docs/basics/vocabulary#config-instance) deployed 
...
`mobility.schema.yaml` specifies the possible values for the `mobility.yaml` config 
instance. On the other hand, `mobility.yaml` defines actual config instance values. 
...
config instances over a unix socket.
