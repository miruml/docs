...
some [config types](/docs/basics/vocabulary#config-type) for a fictional robot. Each config type is 
responsible for a particular aspect of the robot's behavior.

<Frame>
    <img src="/images/core-concepts/config-type-list.png"/>
</Frame>

Every config type has a history of [config schemas](/docs/basics/vocabulary#config-schema) 
as well as a history of [config instances](/docs/basics/vocabulary#config-instance) deployed 
to robots. Below are the config schemas for the `Mobility` config type.
...
`mobility.instance.yaml` supplies config instance values via the local file system for 
quick iteration. `mobility.instance.yaml` is never uploaded to the cloud or deployed to production robots.
...
robot. The agent is a lightweight `systemd` service that serves config instances to the SDK 
over a unix socket.
...
More will be said about the Miru agent in coming sections. For now, the key takeaway is that the development workflow leverages the local file system for consuming config instances via the Miru SDK while the production workflow leverages the Miru agent to serve config instances to the SDK.
...
The CLI collects some git information to aid versioning and tracking config schemas from the Miru dashboard. Miru **does not have access to your git repository**. The git information is extracted from the local git repository where the CLI is run.
