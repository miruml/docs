...
code consuming configurations to minimize versioning errors between code and configurations).
...
## Miru SDK
However, config instances defined in a git repository are for **development use only**. 
...
In production, the SDK retrieves the config instance from the Miru agent running on the 
...
More will be said about the Miru agent in coming sections. For now, the key takeaway is that the development workflow leverages the local file system for consuming config instances via the Miru SDK.
