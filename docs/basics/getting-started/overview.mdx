...
- `configs/`: Contains [config instances](/docs/basics/vocabulary#config-instance) for local development
...
The `/configs/mobility.yaml` defines a config instance adhering to the `Mobility` config schema. This config instance is for development purposes only. Config instances deployed to production robots are defined in the Miru dashboard, not in a git repository.
