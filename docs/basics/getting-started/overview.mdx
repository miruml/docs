...
For the remainder of this guide, the `Mobility` config type will be used as an example.
...
The `/instances/mobility.yaml` defines a config instance adhering to the `Mobility` config schema. This config instance is for development purposes only. Config instances deployed to production robots are defined in the Miru dashboard, not in a git repository.
