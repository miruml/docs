---
title: 'Push Schema'
---

import InstallInstructions from '/snippets/cli-reference/install.mdx'
import LoginInstructions from '/snippets/cli-reference/login.mdx'

### Create the Config Type
To push a schema, it's config type must first be created. Navigate to the [Config Types page](https://configs.miruml.com/configs) in the Miru dashboard.

<Frame>
    <img 
        src="/images/getting-started/create-config-type-page.png" 
        alt="Create Config Type Page" 
    />
</Frame>

Click `+ New Config Type` and name the config type `Mobility` with slug `mobility`.

<Frame>
    <img
        src="/images/getting-started/create-config-type-dialog.png" 
        alt="Create Config Type Dialog" 
    />
</Frame>

<Warning>
    Config Type slugs are permanent and cannot be changed after creation.
</Warning>

### Config Type Slugs
The config type slug is a unique identifier for a config type. It's used by both the CLI and SDK to link a config schema to its corresponding config type.

Thus all config schemas must include the `$miru_config_type_slug` field at the root level. We've
already included this field in `mobility.schema.yaml`.

```yaml mobility.schema.yaml
$miru_config_type_slug: "mobility"
---
```

### Install the CLI

Schemas are pushed via the Miru command-line interface (CLI).

<InstallInstructions />

### Login to the CLI

<LoginInstructions />


### Push to Miru
Navigate back to the root of the [getting-started](https://github.com/miruml/getting-started) 
repository.

```bash
miru schema push ./schemas/mobility.schema.yaml
```

You'll see a confirmation message that your schema was successfully pushed.

```bash
📦 Attempting to push 1 config schemas...

🏁 Successfully pushed schema for mobility config type
    commit: a8340d496013f4e33bb5d59fbebf057a80c44dbf
    branch: main
    origin: https://github.com/miruml/getting-started.git
    git file path: schemas/mobility.schema.yaml


✅ Successfully uploaded 1 config schemas
```

Visit the [Config Types page](https://configs.miruml.com/config-types) and click into the 
`Mobility` config type to verify the schema was uploaded successfully.

<Frame>
    <img 
        src="/images/getting-started/config-schemas-page.png" 
        alt="Successful Schema Push" 
    />
</Frame>

<Warning>
    Git information is pulled from the local git repository. If you don't have access to view the remote repository (specified locally) or the commit hasn't yet been pushed to the remote repository, git related links in Miru will be invalid.
</Warning>
