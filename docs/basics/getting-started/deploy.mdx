...
Once the agent is online, the config instance will move to the `Deployed` state, indicating the config instance has been successfully deployed the target file path on the device.
...
To edit a config instance, simply start making changes to the config instance. As edits are made, a log on the right maintains a list of all the changes made to the config instance.
...
Add a description to the deployment and click **Deploy**.
...
### Archives
You can view a full history of the config instances for a device by clicking the **Deployments** tab in the top left, next to the **Editor** tab. 

<Frame>
   ![](/images/getting-started/device-tabs.png)
</Frame>

Clicking into a config instance from the archive will show the full config instance, the differences from the previously deployed config instance, and useful metadata about the config instance: the current status, who deployed it, when they deployed it, the deployment messages, etc.