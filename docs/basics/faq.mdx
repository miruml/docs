...
- Combines this schema hash with the **device ID** to identify which concrete config to fetch from the cloud.
...
The resulting **concrete config** is then **pulled to the device** by the agent.
