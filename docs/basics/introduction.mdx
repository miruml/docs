...
Robots often require unique configurations for each unit, such as end effector calibrations or camera exposure settings. Given the volume of configurations, associating robots with their correct configurations can be error-prone. 
...
Start with the [core concepts](/docs/basics/core-concepts), which provides a concise overview of Miru's core offering.

If you're ready to start using Miru, still begin with the [core concepts](/docs/basics/core-concepts) before following the [getting started](/docs/basics/getting-started) guide.