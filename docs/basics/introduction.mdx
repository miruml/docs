...
Different groups of robots in your fleet share varying subsets of configurations. For example, robots may be grouped by hardware variant, customer, or test environment. 
...
For example, when a LIDAR sensor is recalibrated, its new configuration is saved on the device first and only backed up to the cloud if necessary. However, since robots are expected to lose network connectivity or power, locally edited configurations cannot always be immediately synced to the cloud.
...
Start with the [core concepts](/docs/basics/core-concepts) guide, which provides a concise overview of Miru's core offering.

If you're ready to use Miru, start with the [core concepts](/docs/basics/core-concepts) before following the [getting-started](/docs/basics/getting-started) guide.