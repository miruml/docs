...
The Miru Agent does not (currently) provide a command line interface. 

The [CLI](/docs/developers/cli/intro) is only for managing Miru resources from your development machine's terminal. Please do not attempt to use the `miru` command to manage the agent.

However, the agent can be managed using standard `systemd` commands and other standard Linux tools.

## Version

To check the version of the agent, use the `dpkg-query` command to query the version of the `miru-agent` debian package.
...
The Miru Agent's logs are stored in the `systemd` journal and can be viewed using the `journalctl` command.

```bash
journalctl -u miru
```

By default, `journalctl` truncates each line's output to the size of the terminal window. To view the full log output for each line, use the `--no-pager` flag.
...
<Tabs >
  <Tab title="Active">
    ...
    <Frame>
      ![](/images/agent/commands/systemctl-status:running.png)
    </Frame>
  </Tab>
  <Tab title="Inactive">
    ...
    <Frame>
      ![](/images/agent/commands/systemctl-status:dead.png)
    </Frame>
  </Tab>
</Tabs>
...
`stop` stops the agent while leaving it's state intact. Stopping the agent does not [disable](/docs/developers/agent/commands#disable) the agent--it will still automatically start on next system boot.
