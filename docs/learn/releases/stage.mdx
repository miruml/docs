...
The concept of deployment drift ensures that configuration changes aren't lost when migrating from one release to another, further preventing misconfigurations.
...
Reviewing a deployment displays the patches that have occurred for a device since the last time that a deployment was reviewed (or when it was initially staged).
...
A dialog will appear with a list of deployments that have been patched to the device since the last time this deployment was reviewed (or when it was initially staged).

<Frame>
  ![](/images/releases/stage/review-dialog:deployments.png)
</Frame>

Clicking into a deployment will open the deployment details panel on the right of the page. You have two options to proceed.
...
<Note>
  The ability to selectively apply patches to a drifted deployment is currently unavailable but will be coming soon!
</Note>