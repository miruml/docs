...
Ideally schemas are versioned in the same repository as the code which uses them. However, this is not a requirement. You may use any git repository to house your schemas.
...
Annotations are a convenient way to store schema metadata with your schema. Most annotations are optional, but some are required for Miru to properly process the schema.
