...
<Framed 
  background="https://assets.miruml.com/docs/v02/images/deployments/background.jpeg"
  image="https://assets.miruml.com/docs/v02/images/deployments/header:panel.png"
...
|---------------|-------------|
| `staged`     | Deployment doesn't yet want to be deployed yet but may be deployed in the future |
...
The error status is independent of the activity and target statuses. The error status does not imply any particular activity or target state and vice versa. The error status is only concerned with any errors encountered during the deployment process.
...
On the other hand, as long as the agent is unable to recover the deployment from the error, the deployment remains in the `retrying` error state. The agent continually re-attempts to fulfill the deployment's target status with an exponential backoff to prevent throttling the device's resources.
