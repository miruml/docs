...
Of course, many deployments skip staging and deploy immediately. The diagram below shows the valid transitions between target statuses for a deployment that is immediately deployed.
...
| `queued`        | Deployment is waiting for the device to be <Tooltip tip="[object Object]">online</Tooltip> so it can be delivered to the device |
