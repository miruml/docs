...
There are 6 possible activity states for a deployment.
...
When a deployment is [staged](/docs/learn/releases/staging-area) before being deployed, there are several activity states it can pass through. Below is a diagram of the valid transitions between activity states.
...
Deployments which are immediately deployed and skip the `staged` state have a simpler lifecycle, passing straight through from `queued` to `archived`.
...
| `retrying`   | An non-fatal error has been encountered; the agent is retrying to fulfill the deployment's target status |
...
On the other hand, while the agent is unable to recover the deployment from the error, the deployment remains in the `retrying` error state. The agent continually attempts to fulfill the deployment's target status with an exponential backoff to prevent throttling the device's resources.
