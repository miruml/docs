...
For example, when [listing devices](/docs/api-reference/server-side/endpoints/devices/list), you can filter by `id`, `name`, and other fields.
...
`id` is a common filter amongst all list endpoints. It allows you to filter by a resource’s unique identifier. For example, when listing devices, you can filter by the [**device id**](/docs/api-reference/server-side/endpoints/devices/list#parameter-id) using **`id=<device-id>`** in the request URL.
...
Filtering by id will return at most one resource, since IDs are unique. Other fields, like [config instance activity status](/docs/api-reference/server-side/endpoints/config-instances/list#parameter-activity-status), can return multiple results.
## Multi-Value Filters
