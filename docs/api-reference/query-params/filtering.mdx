...
For example, when [listing devices](https://docs.miruml.com/docs/api-reference/devices/list), you can filter by `id`, `name`, and other fields.
...
`id` is a common filter amongst all list endpoints. It allows you to filter by a resource’s unique identifier. For example, when listing devices, you can filter by the [**device id**](https://docs.miruml.com/docs/api-reference/devices/list#parameter-id) using **`id=<device-id>`** in the request URL.
...
Filtering by id will return at most one resource, since IDs are unique. Other fields, like [config instance activity status](https://docs.miruml.com/docs/api-reference/config-instances/list#parameter-activity-status), can return multiple results.
## Multi-Value Filters
