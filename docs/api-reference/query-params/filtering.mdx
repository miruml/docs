...
Other fields, such as the config instance [activity status](/docs/api-reference/config-instances/list#parameter-activity-status), are not unique amongst their resources and can therefore return multiple results when being filtered.
