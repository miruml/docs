...
To get the total count, you must explicitly request it using the `expand[]=total_count` parameter. While useful, the total count is typically not needed and can incur significant performance costs so please use judiciously. See the `total_count` [expansion](/docs/api-reference/queries/expansions#total-count) documentation for more information about retrieving the total count. 
