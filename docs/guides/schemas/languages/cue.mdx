...
Miru fully supports CUE and stays up to date with the latest CUE features and syntax. As of writing, Miru uses CUE API [`v0.14.2`](https://github.com/cue-lang/cue/releases/tag/v0.14.2), which is fully supported by Miru.

### File formats

CUE schemas must be written in the CUE file format and have the `.cue` extension.

However, CUE can validate config instances in many different formats. Currently, Miru only supports JSON for config instances, but support for YAML and JSON-C is coming soon!
