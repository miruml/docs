...
[JSON Schema](https://json-schema.org/) is a popular open-source schema language, useful for defining valid fields, types, and values for a config. 

With origins dating back to 2010, JSON Schema was originally built for API validation, but has since found its way into configuration systems across web, cloud, and robotics infrastructure.

This section covers key integration points for JSON Schema with Miru as well as some general information on the strengths and weaknesses of JSON Schema.
...
Miru supports the full suite of keywords for Draft 2020-12.
...
The format of a schema does not need to match the format of the instance it validates. For instance, a YAML schema can validate a JSON instance.
