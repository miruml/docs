...
The configuration language used to define the schema. 

Currently supported configuration languages include:
...
The raw schema definition written in the specified configuration language.
...
The following is a toy schema defined using the JSON Schema [configuration language](/docs/guides/schemas/languages).
...
While some configurations, such as calibrations, cannot provide sensible default values, defaults should be leveraged for the majority of parameters.
...
Miru integrates directly with default values in your schemas. When creating a new config instance, Miru automatically populates the instance with the defaults provided by the schema.

Importantly, Miru maintains the structural integrity of the schema when populating default instances--the order of properties in the default instance is identical to its schema. This is particularly useful for large config instances where it can be difficult to recall the exact placement of a property and navigate to it effectively.
