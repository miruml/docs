...
Support for additional configuration languages is planned for the future, including Pkl, Type Schema, JSON Type Definition, and Dhall.
...
[CUE](https://cuelang.org/) is a powerful configuration language with built-in validation, type checking, and constraint capabilities. It combines the benefits of JSON with the expressiveness of a type system.
...
Miru supports schemaless schemas--empty schemas that regard all config instances as valid. Schemaless schemas are useful to quickly begin using Miru, rapid prototyping, or when opting to use custom validation instead of configuration language.

Below are the schemaless schemas for the supported configuration languages.
