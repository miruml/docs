...
Provisioning devices is the process of creating a device in Miru and installing the Miru Agent onto the device. Once complete, the device is ready to receive 
...
The dashboard is particularly useful for initial setup, small deployments, development environments, or when you need the visual feedback and management capabilities of the dashboard.
...
The file path to the Miru Agent debian package to use for installation. Useful for installing the agent in [poor connectivity scenarios](/docs/agent/install/poor-connectivity).
</ParamField>
...
Some environments may have particularly poor connectivity. Whether located in a remote area or using a cellular connection, downloading the Miru Agent over a network may be impractical in some environments.
