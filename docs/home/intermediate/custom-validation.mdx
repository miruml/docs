...
Upon any new config instance deployments made in the UI (or otherwise), Miru will trigger the [validate deployment webhook](/docs/api-reference/webhooks/events/deployments/validate) to your endpoint(s). 

The validatin results of the deployment must be sent to the [validate deployment API](/docs/api-reference/server-side/endpoints/deployments/validate). If the deployment is marked as invalid, it is immediately archived and marked as `failed`. If the deployment is marked as valid, it continues to its desired target status (i.e. `pending`, `approved`, or `deployed`).
