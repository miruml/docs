...
The first step in defining a config is creating its config type. Navigate to the [Config Types page](https://configs.miruml.com/configs).
...
All config schemas must include the `$miru_config_type_slug` field at the root level. We've already included this field in `mobility.schema.yaml` and `empty-mobility.schema.yaml`.
...
Schemas are created via the Miru command-line interface (CLI). To push a schema, you'll need to install the CLI on your development machine.
