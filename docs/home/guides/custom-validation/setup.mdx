...
Miru natively supports the use of [JSON Schema](https://json-schema.org/) to validate configurations. However, you may find it more convenient to validate configurations with your own software stack instead of using JSON Schema.

To support this, Miru provides [webhooks](/docs/api-reference/webhooks/events/deployments/validate) and [server-side APIs](/docs/api-reference/server-side/endpoints/deployments/validate).

You host your own validation server, which is notified of new deployments via a webhook. The server then validates the configurations with the Miru API before the deployment can continue.

This guide walks through the steps of setting up a validation server to integrate with Miru.


## Enable
Custom validation is enabled on a per-release basis.

To enable custom validation for a release, navigate to the [Releases](https://configs.miruml.com/releases) page. Click the ellipses on the far right of the config type you would like to enable and select **Edit** from the dropdown.
...
<Warning>
    Enabling custom validation *requires* you to receive the validation webhook and validate deployments via APIs for that release. If these APIs are not implemented, deployments may remain in the `validating` state indefinitely.
</Warning>
