...
<Warning>
  Enabling custom validation _requires_ deployments to be validated. If you do not have a validation server set up, deployments on this release will be unable to deploy.
</Warning>
...
We recommend storing secrets in a secrets manager or other secure location.

No matter where you choose to store them, be sure not to expose these secrets in publicly accessible areas such as GitHub, client-side code, and so forth.
...
The [validate deployment API](/docs/api-reference/server-side/endpoints/deployments/validate) response contains an `effect` field, which returns the effect of validating the deployment. Depending on the validation results and the user's intent, the deployment may be launched into one of several different states as a result. The `effect` field records what the deployment's next state is, and the `message` field explains why.
