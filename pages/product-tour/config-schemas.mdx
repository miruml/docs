...
## Define your config schemas

Miru requires you to define a config schema for each of your [configs](/pages/core-concepts#config). These schemas are used to validate concrete configs before they are deployed, effectively preventing runtime application errors. These schemas live in your git repository (preferably in the same repository as the code consuming the config) to minimize versioning errors between code and configurations.
...
<img src="/images/product-tour/motion-control-schema-list.png" style={{borderRadius: '7px'}}/>
