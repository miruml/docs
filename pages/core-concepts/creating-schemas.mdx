...
Miru requires you to define a config schema to deploy a config. These schemas are used to validate concrete configs before they are deployed, effectively preventing runtime application errors. Config schemas **live in your git repository** (preferably in the same repository as the code consuming the config) to minimize versioning errors between code and configurations.
