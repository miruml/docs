...
## Local Development 
Miru provides an [SDK](/pages/guides/sdk) for deploying concrete configs to your devices in production. We'll cover this more in depth later but the key point to know here is that the SDK pulls concrete configs from the cloud.
...
However, it's critical that an SDK allows you to easily iterate through concrete config values in your local development environment as well. The SDK accepts a boolean argument to determine whether it should use a file on the local file system (development) or fetch values from the cloud (production).
...
This allows you to quickly iterate through values in development using a file on your filesystem while providing the same interface for fetching config values from the cloud in production.

It's important to note that, although not recommended, you are not required to use the config values returned by the Miru SDK in production either. Instead you can tell the SDK to update a file on your filesystem with the desired config. 
