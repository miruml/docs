...
func (c *ArtifactsAPIController) Routes() Routes {
	return Routes{
		...
		"ArtifactsArtifactIdDeploymentsGet": Route{
			strings.ToUpper("Get"),
			"/internal/v1/artifacts/{artifact_id}/deployments",
			c.ArtifactsArtifactIdDeploymentsGet,
		},
...
func (c *ArtifactsAPIController) ArtifactsArtifactIdDeploymentsGet(w http.ResponseWriter, r *http.Request) {
	params := mux.Vars(r)
	artifactIdParam := params["artifact_id"]
	if artifactIdParam == "" {
		c.errorHandler(w, r, &RequiredError{"artifact_id"}, nil)
	...
	result, err := c.service.ArtifactsArtifactIdDeploymentsGet(r.Context(), artifactIdParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
